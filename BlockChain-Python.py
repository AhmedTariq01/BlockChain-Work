...
class Block:
    ...
    def add_transaction(self, sender, receiver, amount):
        ...
        return previous_block['index'] + 1 # index of bock that will going to be added in chain 
    ...
    def replace_chain(self):
        network = self.nodes #here p2p network is all set of nodes placed in node set
        longest_chain = None
...
def add_transaction():
    json = request.get_json() # get Post method parameters in JOSN format
...
def connect_node():
    json = request.get_json() # get addresses of nodes from post request
    nodes = json.get('nodes')
    if nodes is None:
        return "No node", 400
    for node in nodes:
        blockchain.add_node(node) # add node URLs in node set
